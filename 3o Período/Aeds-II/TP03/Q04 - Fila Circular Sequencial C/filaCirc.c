...
struct FilaCirc {
    ...
    struct Character characterList[TAM_FILA+1];
    int primeiro, ultimo;
}; // struct FilaCirc
...
int mediaIdade(struct FilaCirc *fila){
    int soma = 0, numElem = 0;

    for(int i = fila->primeiro; i != fila->ultimo; i = ((i + 1) % (TAM_FILA+1))){
        numElem++;
        soma += fila->characterList[i].yearOfBirth;
    }

    int media = soma/numElem;
    return media;
}// mediaIdade()
...
struct Character remover(struct FilaCirc *fila){
    ...
    struct Character removido = fila->characterList[fila->primeiro];
...
void inserir(struct FilaCirc *fila, struct Character newCharacter){
    ...
    fila->characterList[fila->ultimo] = newCharacter;
...
void printFila(struct FilaCirc *fila){
    int i = fila->primeiro, j = 0;
    
    printf("[ Head ]\n");
    while(i != fila-> ultimo){
        printf("[%i ## ", j);
        printCharacter(fila->characterList[i]);
        printf("]\n");
