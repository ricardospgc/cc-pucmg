...
void insercaoPorCor(Character *list, int size, int cor, int h){
    for (int i = (h + cor); i < size; i += h) {
...
// Shellsort por eyeColor
void shellsort(Character *list, int size) {
    int h = 1;
    ...
    do {
        ...
        for(int cor = 0; cor < h; cor++){
            insercaoPorCor(list, size, cor, h);
        }
    } while (h != 1);
}// shellsort()
...
void criarLog(){
    FILE *arq = fopen("803833_shellsort.txt", "w");
    if (arq == NULL) {
