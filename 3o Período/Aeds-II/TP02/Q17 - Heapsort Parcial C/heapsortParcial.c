...
// Compara dois atributos dos personagens, desempatando pelo nome
...
// constroi o Heap
void construir(Character tmp[], int tamHeap){
    for(int i = tamHeap; i > 1 && cmpCharacter(tmp[i], tmp[i/2]) > 0; i /= 2){
        swap(&tmp[i], &tmp[i/2]);
    }
}// construir()

// retorna o index do maior filho
int getMaiorFilho(Character tmp[], int i, int tamHeap){
    int filho = 0;
    if (2*i == tamHeap || (cmpCharacter(tmp[2*i], tmp[2*i+1]) > 0)){
        filho = 2*i;
    } else {
        filho = 2*i + 1;
    }
...
// reconstroi o array, ordenando
void reconstruir(Character tmp[], int tamHeap){
    int i = 1;
    while(i <= (tamHeap/2)){
        int filho = getMaiorFilho(tmp, i, tamHeap);
        if(cmpCharacter(tmp[i], tmp[filho]) < 0){
...
// Prepara o Array temporario e controla o fluxo de Heapsort
void heapsort(Character* list, int size, int k) {
    Character tmp[size+1];

    //cria tmp com shift para a direita, liberando a ultima pos
...
void sort(Character *list, int size, int k){
    
    clock_t start = clock();

    heapsort(list, size, k);
...
void criarLog(){
    FILE *arq = fopen("803833_heapsortParcial.txt", "w");
    if (arq == NULL) {
...
int main(){
    ...
    sort(registro.characterList, registro.size, k);
