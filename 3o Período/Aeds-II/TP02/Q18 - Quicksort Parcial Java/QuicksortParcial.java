...
class Registro {
    ...
    /**
     * Printa os K primeiros da lista
     */
    public void printCharacterList(int k){
        for(int i = 0; i < k; i++){
            characterList.get(i).printCharacter();
        }
    }// printCharacterList()
     ...
     * Printa somente um atributo de todos os personagens da lista
     */
    public void printListAttribute(int k){
        for(int i = 0; i < k; i++){
            MyIO.println(characterList.get(i).getName() + ": " + characterList.get(i).getDateOfBirth());
        }
    }// printListAttribute()
...
public class QuicksortParcial{
    ...
    public static String logFileName = "803833_quicksortParcial.txt";
     ...
     */
    public static void quicksortParcial(ArrayList<Character> list, int esq, int dir, int k){
        int i = esq, j = dir;
        Character pivo = list.get((dir+esq)/2);
        while (i <= j) {
            while (cmpCharacter(list.get(i), pivo) < 0) i++;
            while (cmpCharacter(list.get(j), pivo) > 0) j--;
            if (i <= j) {
                swap(list, i, j);
                i++;
                j--;
        ...
        if (esq < j)  quicksortParcial(list, esq, j, k);
        if (i < dir && i < k)  quicksortParcial(list, i, dir, k);
    }// insertion()
     ...
     * @param registro
     */
    public static void sort(Registro registro, int k){
        Instant start = Instant.now();

        quicksortParcial(registro.characterList, 0, registro.characterList.size()-1, k);
