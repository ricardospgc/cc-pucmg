...
public class Heapsort{
    ...
    public static String logFileName = "803833_heapsort.txt";
    ...
    public static int cmpCharacter(Character a, Character b){
        int cmp = a.getHairColor().compareTo(b.getHairColor());
        numComparacoes++;
     ...
     * @param list
     */
    public static void heapsort(ArrayList<Character> list) {
        //Alterar o vetor ignorando a posicao zero
        list.add(0, new Character());
  
        //Contrucao do heap
        for(int tamHeap = 2; tamHeap <= list.size()-1; tamHeap++){
           construir(list, tamHeap);
        }
  
        //Ordenacao propriamente dita
        int tamHeap = list.size()-1;
        while(tamHeap > 1){
           swap(list, 1, tamHeap--);
           reconstruir(list, tamHeap);
        }
     ...
     * @param tamHeap
     */
    public static void construir(ArrayList<Character> list, int tamHeap){
        for(int i = tamHeap; (i > 1) && (cmpCharacter(list.get(i), list.get(i/2)) > 0); i /= 2){
            swap(list, i, i/2);
        }
    }// construir()
      ...
      * @param tamHeap
      */
    public static void reconstruir(ArrayList<Character> list, int tamHeap){
        int i = 1;
        while(i <= (tamHeap/2)){
            int idxFilho = getIdxMaiorFilho(list, i, tamHeap);

            if(cmpCharacter(list.get(i), list.get(idxFilho)) < 0){
                swap(list, i, idxFilho);
                i = idxFilho;
            }else {
                i = tamHeap;
            }
    ...
    /**
     * retorna o index do maior filho
     * @param i
     * @param tamHeap
     * @return
     */
    public static int getIdxMaiorFilho(ArrayList<Character> list, int i, int tamHeap){
        int filho;
        if (2*i == tamHeap || cmpCharacter(list.get(2*i), list.get(2*i+1)) > 0){
            filho = 2*i;
        } else {
            filho = 2*i + 1;
        }
    ...
    public static void sort(Registro registro){
        ...
        heapsort(registro.characterList);
