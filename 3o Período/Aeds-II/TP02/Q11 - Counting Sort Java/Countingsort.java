...
public class Countingsort{
    ...
    public static void countingSort(ArrayList<Character> list){
        ...
        //Inicializar cada posicao do list de contagem 
        for (int i = 0; i < count.length; count[i] = 0, i++);

        //Agora, o count[i] contem o numero de elemento iguais a i
        for (int i = 0; i < list.size(); count[list.get(i).getYearOfBirth()]++, i++);
        ...
        ordenado[count[list.get(i).getYearOfBirth()]-1] = list.get(i).clone(), 
        count[list.get(i).getYearOfBirth()]--, i--, numMovimentacoes++);

        //Copiando para o list original
        for(int i = 0; i < list.size();  list.set(i, ordenado[i]), i++, numMovimentacoes++);
    ...
    public static int getMaior(ArrayList<Character> list, int tam) {
        ...
        for (int i = 0; i < tam; i++) {
            numComparacoes++;
            if(maior < list.get(i).getYearOfBirth()) maior = list.get(i).getYearOfBirth();
        }
        return maior;
    }// getMaior()
    ...
    public static void sort(Registro registro){
        ...
        countingSort(registro.characterList);
