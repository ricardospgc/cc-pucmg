...
void radixsort(Character* list, int size, int n) {
    ...
    for (int pos = n - 1; pos >= 0; pos--) {
        ...
        for (int i = 0; i < size; i++) {
            count[(unsigned char)list[i].id[pos]]++;
        }
        for (int i = 1; i < 256; i++) {
            count[i] += count[i - 1];
        }
        // array auxiliar para armazenar os personagens ordenados temporariamente
        Character *temp = (Character *)malloc(size * sizeof(Character));
        for (int i = size - 1; i >= 0; i--) {
            temp[count[(unsigned char)list[i].id[pos]] - 1] = list[i];
            count[(unsigned char)list[i].id[pos]]--;
        }
        for (int i = 0; i < size; i++) {
            list[i] = temp[i];
...
void sort(Character *list, int size){
    ...
    clock_t start = clock();
    int max = strlen(list[0].id);
    radixsort(list, size, max);
...
void criarLog(){
    FILE *arq = fopen("803833_radixsort.txt", "w");
    if (arq == NULL) {
...
int main(){
    Registro registro;
    newRegistro(&registro, 1); // 0 = linux / 1 = Windows / 2 = VERDE
