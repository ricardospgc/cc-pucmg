...
class Registro {
    ...
    public void printListAttribute(){
        characterList.forEach(n -> { MyIO.println(n.getName()) ;});
    }// printListAttribute()
...
public class SelectionParcial{
    ...
    public static String logFileName = "803833_selecaoParcial.txt";
    ...
    /**
     * Ordena o array usando Selection Sort, pelo nome do personagem
     * @param list ArrayList de personagens
     */
    public static void selectionParcial(ArrayList<Character> list, int k){
        for (int i = 0; i < k; i++) {
			int menor = i;
			for (int j = (i + 1); j < list.size(); j++){
				if (list.get(menor).getName().compareTo(list.get(j).getName()) > 0){
					menor = j;
				}
                numComparacoes++;
			}
    ...
    public static void sort(Registro registro, int k){
        ...
        selectionParcial(registro.characterList, k);
