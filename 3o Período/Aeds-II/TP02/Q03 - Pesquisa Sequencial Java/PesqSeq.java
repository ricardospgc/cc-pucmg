...
public class PesqSeq{
    ...
    public static void criarLog(){
        try {
            ...
            writer.write("Matricula: 803833\tTempo: " + time + "ms\t Comparações: " + numComparacoes);
            writer.close();
        } catch (IOException e) {
    ...
    /**
     * Pesquisa Sequencial pelo nome do personagem
     * @param arrC
     * @param nameKey
     * @return boolean true se achou, falso se nao
     */
    public static boolean pesqSeq(ArrayList<Character> characterList, String nameKey){
        boolean resp = false;
        int n = characterList.size();
          
        for(int i = 0; i < n; i++){
            if(characterList.get(i).getName().equals(nameKey)){
                resp = true;
                i = n;
            }
            numComparacoes++;
     ...
     * @return boolean true se achou, falso se nao
     */
    public static boolean pesquisa(String nameKey, Registro registro){
        
        Instant start = Instant.now();
        boolean resp = pesqSeq(registro.characterList, nameKey);
    ...
    static public void main(String[] args){
        ...
        while(!isFim(nameKey)){
            MyIO.println(pesquisa(nameKey, registro) ? "SIM" : "NAO");
            nameKey = scn.nextLine();
        }
