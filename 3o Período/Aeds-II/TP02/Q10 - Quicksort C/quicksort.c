...
void printListAttribute(Registro registro){
    for(int i = 0; i < registro.size; i++){
        printf("%s: %s\n", registro.characterList[i].name, registro.characterList[i].eyeColor);
    }
}// printListAttribute()
...
void quicksort(Character *list, int esq, int dir) {
    int i = esq, j = dir;
    Character pivo = list[(dir+esq)/2];
    while (i <= j) {
        while (cmpCharacter(list[i], pivo) < 0) i++;
        while (cmpCharacter(list[j], pivo) > 0) j--;
        if (i <= j) {
            swap(list + i, list + j);
            i++;
            j--;
    ...
    if (esq < j)  quicksort(list, esq, j);
    if (i < dir)  quicksort(list, i, dir);
}// quicksort()
...
void criarLog(){
    FILE *arq = fopen("803833_quicksort.txt", "w");
    if (arq == NULL) {
