...
void printListAttribute(Registro registro){
    for(int i = 0; i < registro.size; i++){
        printf("%s\n", registro.characterList[i].name);
    }
}

/***** QUESTAO ******/

int minIndex(Character *list, int size, int idx){
    int resp = idx;
    if(idx != size){
        int min = minIndex(list, size, idx + 1);
        
        numComp++;
        if(strcmp(list[min].name, list[idx].name) < 0)
            resp = min;
        else
            resp = idx;
    }
...
// Ordenacao por selecao recurssiva
void selectionRec(Character *list, int size, int idx){
    if(idx != size){
        int min = minIndex(list, size-1, idx);

        if(min != idx) 
            swap(&list[idx], &list[min]);
        
        selectionRec(list, size, idx + 1);
    }
}// selectionRec()
...
void criarLog(){
    FILE *arq = fopen("803833_selecaoRecursiva.txt", "w");
    if (arq == NULL) {
