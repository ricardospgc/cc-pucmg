...
public class Hash {
    ...
    public static String logFileName = "803833_hashReserva.txt";
    ...
    Hash(int option){
        ...
        this.hash = new Character[tamTotal];
    ...
    /**********/
    /**
     * Transforma o elemento em uma hash
     * @param elemento
     * @return hash do elemento
     */
     ...
     * @param elemento
     */
    public void inserir(Character elemento) {
        int pos = h(elemento.getName());

        if (hash[pos] == null) {
            hash[pos] = elemento;

        } else if (posReserva < tamReserva) {
            hash[tamTab + posReserva] = elemento;
            posReserva++;
        }
    }// inserir()

    /**
     * Pesquisa um elemento na tabela pelo nome
     * @param key a ser pesquisada
     * @return a posicao do elemento
     */
    public int pesquisar(String key) {
        int pos = h(key);
        int resp = -1;
        
        if (hash[pos] != null && hash[pos].getName().equals(key)) {
            numComparacoes++;
            resp = pos;
            ...
            for (int i = 0; i < posReserva; i++) {
                numComparacoes++;
                if (hash[tamTab + i].getName().equals(key)) {
                    resp = tamTab+i;
                    i = posReserva;
                }
            }
    ...
    static public void main(String[] args){
        ...
        Hash tabela = new Hash(); // 0 = linux / 1 = Windows / void = VERDE
        ...
        try{
            ...
            while(!isFim(key)){
                ...
                int pos = tabela.pesquisar(key);
                MyIO.println((key + ((pos == -1) ? " NAO" : " (Posicao: "+ pos +") SIM")));
